...
func (d *DHT) Start() (err error) { //dht 启动入口函数
	fmt.Println("start")
	if err = d.initSocket(); err == nil { //启动监听udp服务
		d.wg.Add(1) //WaitGruoup加1 柱塞主线程结束以等待一个goroutine结束
		go func() { //上面设置的应该是等待这一个 goroutine
			defer d.wg.Done() //goroutine结束后 放弃柱塞
			d.loop()          //主循环
		}()
		d.wg.Wait()
	}
...
func (d *DHT) initSocket() (err error) {
	...
	d.config.Port = d.conn.LocalAddr().(*net.UDPAddr).Port //反馈真实的监控端口 有可能你设置的是0 为自动选取
	return nil
}

func (d *DHT) bootstrap() {
	// Bootstrap the network (only if there are configured dht routers).
	for _, s := range strings.Split(d.config.DHTRouters, ",") { //传入的路由表
		if s != "" {
			d.ping(s)                                                        //是否能ping通
			r, e := d.routingTable.getOrCreateNode("", s, d.config.UDPProto) //查找Node信息没有时候添加 先把对方加入到自己的K同中
			if e == nil {
				d.findNodeFrom(r, d.nodeId) //通知远程节点 查询本节点 以上本节点注册到对方的路由表中
			}
		}
	}
	d.findNode(d.nodeId)
...
func (d *DHT) loop() {
	...
	bytesArena := newArena(maxUDPPacketSize, 3) //预先申请内存支持弹出压入的 chan []byte
	socketChan := make(chan packetType)         //无缓存的 包结构chan
	d.wg.Add(1)
	go func() {
		defer d.wg.Done()
		readFromSocket(d.conn, socketChan, bytesArena, d.stop) //处理udp接受的信息
	}()
	fmt.Println("bootsrap")
	d.bootstrap() //启动加入DHT网络

	cleanupTicker := time.Tick(d.config.CleanupPeriod)  //应该是24小时候无更新清楚peers信息
	secretRotateTicker := time.Tick(secretRotatePeriod) //刷新路由表

	saveTicker := make(<-chan time.Time) //多长时间持久化一下路由表
	if d.store != nil {
...
func (d *DHT) processPacket(p packetType) {//处理接受包
...
func (d *DHT) pingNode(r *remoteNode) {
	...
	sendMsg(d.conn, r.address, query) //udp ping节点
	totalSentPing.Add(1)
}
...
func (d *DHT) replyAnnouncePeer(addr net.UDPAddr, node *remoteNode, r responseType) {//其他节点通知开始下载资源
